...
const removePreference = (destination: string) => browser.runtime
  .sendMessage<DestinationPayload, BackgroundTaskResult>({ action: ActionName.DeleteAccountAction, destination })
...
const htmlUtils = {
  create: (elemName: string, text = '', children = [] as any) => {
  ...
  createTable: (tableRows: any) => {
    ...
    const tableHeaderContainer = htmlUtils
      .create('tr', '', [loginDestinationHeader, preferredAccountHeader, emptyHeader])
